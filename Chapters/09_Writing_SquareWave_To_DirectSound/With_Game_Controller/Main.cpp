                     ...
                     _In_ int ShowCode)
{
    ...
    if (RegisterClassA(&WindowClass))
    {
        ...
        if (Window)
        {
            ...
            int32 Frequency = 432; // Natural Frequency
            // int32 Frequency = 440; // Modern Standard Musical Frequency
            int32 Samples_Per_Second = 48'000;
            int16 Sound_Volume = 2000;
            int32 SquareWave_Period = Samples_Per_Second / Frequency;
            ...
            Win32_Init_DirectSound(Window, Samples_Per_Second, Secondary_Buffer_Size);
            // Play Sound
            ...
            while (Global_Running)
            {
                ...
                Render_Some_Weird_Gradient(&Global_Back_Buffer, OffsetX,
                                           OffsetY);
                ...
                DWORD Bytes_To_Lock = (Running_Sample_Index * Bytes_Per_Sample) % Secondary_Buffer_Size;
                DWORD Bytes_To_Write;

                if (Bytes_To_Lock == Play_Cursor) {
                    Bytes_To_Write = Secondary_Buffer_Size;
                } else if (Bytes_To_Lock > Play_Cursor) {
                    Bytes_To_Write = (Secondary_Buffer_Size - Bytes_To_Lock) + Play_Cursor;
                } else {
                ...
                {
                    int16 SampleValue = ((Running_Sample_Index++ / (SquareWave_Period/2)) % 2)
